...
class Dynamics():
    def __init__(self, urdf_file: str, initialize: bool = False, initial_angles: List = [], robot_object = None) -> None:
        '''
        ...
        self.rel_path = os.path.join(self.module_dir, urdf_file)
        self.urdf_file = os.path.abspath(self.rel_path)
    ...
    def initialize_model(self) -> None:
        ...
        for attr in sym_attributes:
            cache_name = f"{self.module_dir}/dynamics/{self.urdf_file.split('.')[-2].split('/')[-1]}_{attr}.pkl"
            if not os.path.isfile(f"{cache_name}"): # Model is not cached
                # Compute symbolic matrix
                getattr(self, f"__{attr}_compute")()
                # Save file in cache
    ...
    def get_mass_matrix(self, joint_angles: List) -> np.ndarray:
        if self._know_params_node == False:
            Exception("The robot parameters have not been initialized yet. \
    ...
    def get_jacobian_matrix(self, joint_angles: List) -> np.ndarray:
        ...
        '''
        if self._know_params_node == False:
            Exception("Warning: The robot parameters have not been initialized yet. \
                       Run Dynamics.initialize_model(joint_angles [rad]) to initialize model.")
    ...
    def __load_cached_model(self, cache_name: str, attribute: str):
        with open(f"{cache_name}", "rb") as f:
            expr = pickle.load(f)
        # Parameterize and store matrix as a function
        lambda_function = sym.lambdify(self.sym_q, expr, modules='numpy')
        setattr(self, attribute, lambda_function)
    ...
    def __sym_R_total_compute(self):
        # Update rotation matrix list
        self.__sym_R_compute()
       
        for i in range(self.num_joints):
            self.sym_R_total = self.sym_R_total @ self.sym_R[i]
    ...
    def __sym_T_total_compute(self):
        # Update transformation matrix list
        self.__sym_T_compute()
        # Combine for total transformation matrix
        for i in range(self.num_joints):
            self.sym_T_total = self.sym_T_total * self.sym_T[i]
    ...
    def __sym_transformation_matrix(self, i: int, q: sym.Symbol) -> np.ndarray:
        ...
        '''
        T_a = sym.eye(4)
        T_a[0,3] = self.params_node.dh_dict['r'][self.joint_shift+i]
        T_d = sym.eye(4)
        T_d[2,3] = self.params_node.dh_dict['d'][self.joint_shift+i]

        # alpha and theta saved in degrees
        alpha = np.deg2rad(self.params_node.dh_dict['alpha'][self.joint_shift+i])
